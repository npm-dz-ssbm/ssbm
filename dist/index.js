...
import { Character as SlpChar, GameEndMethod as GameEndMethodImport, GameMode as GameModeImport, SlippiGame, } from "@slippi/slippi-js";
...
const charRecordRowGetter = (recName) => (k) => charRecordGetter(recName)()[k] || Character.Invalid;
...
export const Character = {
    ...
    GameAndWatch: ssbmChar(SlpChar.GAME_AND_WATCH, "Mr. Game & Watch", "GAME_AND_WATCH", { meleeCSPFilename: "Mr. Game and Watch" }),
...
export const SlpDataMark = $.T.object({
    process: $.T.string(),
    start: $.T.number(),
    length: $.T.number(),
    type: $.T.string(),
...
export const SlpDataPort = $.T.object({
    port: $.T.number(),
...
const ParseError = $.T.defVariant("@dz-ssbm/ssbm|ParseError", {
    NoIdent: $.T.undefined(),
});
const ALL_PLAYER_INDS = [0, 1, 2, 3];
...
export function parseSlpData(bytes) {
    return $.exec(function* () {
        ...
        const matchInfo = $.xMaybe(function* (bind) {
            ...
            const tiebreakerNumber = yield* bind($.maybeFromNullable(info.tiebreakerNumber));
        ...
        const playerIndSet = new Set(ALL_PLAYER_INDS.filter((ind) => Boolean(metadataPlayers[ind]) || Boolean(settingsPlayers[ind])));
        const numPlayers = playerIndSet.size;
        const areAllPlayersParsed = [...playerIndSet].every((ind) => Boolean(settingsPlayers[ind]));
        const sessionName = settings?.matchInfo?.sessionId || "";
        const startAt = $.mapMaybe($.maybeFromNullable(metadata.startAt), (s) => new Date(s).valueOf());
        function infoIdStr(i) {
            const s = randomSeed;
        ...
        const lastFrame = $.orMaybe_($.maybeFromNullable(metadata?.lastFrame), () => Math.max(...Object.keys(frames).map((s) => parseInt(s, 10))));
        const gameEnd = game.getGameEnd();
        function getPlayerType(ind) {
        ...
        function getCC(ind) {
            return $.snullable(settingsPlayers[ind]?.connectCode || metadataPlayers[ind]?.names?.code);
        }
        function getDisplayName(ind) {
            return $.snullable(settingsPlayers[ind]?.displayName ||
                metadataPlayers[ind]?.names?.netplay);
        }
        function getInGameTag(ind) {
        ...
        return {
            ...
            sessionTiebreakerNumber: $.nullableFromMaybe(matchInfo)?.tiebreakerNumber || null,
                ...
                return [
                    {
                        ...
                        groundTechAway: $.nullable(actionByIndex[ind]?.groundTechCount?.away),
                        groundTechIn: $.nullable(actionByIndex[ind]?.groundTechCount?.in),
                        groundTechNeutral: $.nullable(actionByIndex[ind]?.groundTechCount?.neutral),
                        groundTechFail: $.nullable(actionByIndex[ind]?.groundTechCount?.fail),
                        ...
                        edgeCancel: $.nullable(actionByIndex[ind]?.edgeCancelCount?.success),
                        edgeCancelSlow: $.nullable(actionByIndex[ind]?.edgeCancelCount?.slow),
                        ...
                        inputsButtons: $.nullable(overallByIndex[ind]?.inputCounts?.buttons),
                        inputsTriggers: $.nullable(overallByIndex[ind]?.inputCounts?.triggers),
                        inputsCstick: $.nullable(overallByIndex[ind]?.inputCounts?.cstick),
                        inputsJoystick: $.nullable(overallByIndex[ind]?.inputCounts?.joystick),
                        ...
                        dipm: $.nullable(overallByIndex[ind]?.digitalInputsPerMinute?.ratio),
                        opk: $.nullable(overallByIndex[ind]?.openingsPerKill?.ratio),
                        neutralRate: $.nullable(overallByIndex[ind]?.neutralWinRatio?.ratio),
                        counterRate: $.nullable(overallByIndex[ind]?.counterHitRatio?.ratio),
                        tradeRate: $.nullable(overallByIndex[ind]?.beneficialTradeRatio?.ratio),
                    },
                ];
            }),
        };
